...
The NetworkManager and resolved paths are both needed: globally overriding NetworkManager's resolv.conf list alone does not replace the per-link DNS sent to systemd-resolved. Reloading NetworkManager's DNS configuration, then restarting resolved, clears the previous DHCP servers without disconnecting Wi-Fi. No connection profiles, routes, firewall rules or resolv.conf symlinks are edited by this plugin. Disabling removes only this plugin's overrides and reloads the existing DNS configuration. IPv4 and IPv6 are both covered; unfiltered fallback servers are not added.
